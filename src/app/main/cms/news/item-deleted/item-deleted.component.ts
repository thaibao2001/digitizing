...
export class ItemDeletedComponent extends Grid implements OnInit {
  public auto_fill = false;
  public isCreate = false;
  public item: Item;
  public selectedItemGroupSearch: any;
  public item_groups: any;
  public hasUpdateReoveermission: boolean;
  public hasUpdateUndoermission: boolean;
  ...
  public constructor(injector: Injector) {
    ...
    this.hasSearchAllPermission = this._authenService.hasPermission(this.pageId, 'search_item_del_all');
    this.hasViewPermission = this._authenService.hasPermission(this.pageId, 'view_item_pub');
    this.hasUpdateReoveermission = this._authenService.hasPermission(this.pageId, 'remove_item');
    this.hasUpdateUndoermission = this._authenService.hasPermission(this.pageId, 'update_item_undo');
    ...
    this.predicateBeforeSearch = () => {
      ...
      this.searchFormGroup.get('item_status_rcd').setValue('DEL');
  ...
  public onUndo(row) {
    this._translateService.get('MESSAGE.confirm_undo').subscribe((message) => {
      this._confirmationService.confirm({
        message: message,
        accept: () => {
          this._apiService.post('/api/adapter/execute', { Method: { Method: 'POST' }, Url: '/api/item/update-item-undo', Module: 'CMS', Data: JSON.stringify(row) }).takeUntil(this.unsubscribe).subscribe(res => {
            this.searchFormGroup.get('item_group_id').setValue(this.selectedItemGroupSearch);
            this.search();
  ...
  public onRemove(items: any[]) {
    if (items.length > 0) {
      this._translateService.get('MESSAGE.confirm_remove').subscribe((message) => {
        this._confirmationService.confirm({
          message: message,
          accept: () => {
            let removeIds = [];
            ...
            if (removeIds.length > 0) {
              this._apiService.post('/api/adapter/execute', { Method: { Method: 'POST' }, Url: '/api/item/remove-item', Module: 'CMS', Data: JSON.stringify(removeIds) }).takeUntil(this.unsubscribe).subscribe(res => {
                this.searchFormGroup.get('item_group_id').setValue(this.selectedItemGroupSearch);
                this.search();
