...
export class ViewSubjectComponent extends Grid implements OnInit {
  ...
  public openCreateModal(row: any = null) {
    ...
    setTimeout(() => {
      ...
      this.updateForm = new FormGroup({
        tag_name_l: new FormControl('', [
          Validators.required,
          Validators.maxLength(1000),
        ]),
        tag_name_e: new FormControl('', [
          Validators.required,
          Validators.maxLength(1000),
        ]),
  ...
  public onSubmit() {
    if (this.submitting == false) {
      ...
      if (
        !this.website_tag.tag_description_l &&
        this.website_tag.tag_description_e
      ) {
      ...
      if (this.isCreate) {
        this._apiService
          .post('/api/adapter/execute', {
            Method: { Method: 'POST' },
            Url: '/api/website-tag/create-website-tag',
            Module: 'CMS',
            Data: JSON.stringify(this.website_tag),
          })
          .subscribe(
            (res) => {
              ...
              if (
                this._storageService.getItem(
                  SystemConstants.get('PREFIX_DATA_LANGUAGE')
                ) == 'en'
              ) {
        ...
        this._apiService
          .post('/api/adapter/execute', {
            Method: { Method: 'POST' },
            Url: '/api/website-tag/update-website-tag',
            Module: 'CMS',
            Data: JSON.stringify(this.website_tag),
          })
          .subscribe(
            (res) => {
              let index = this.data.findIndex(
                (ds) => ds[this.dataKey] == this.website_tag[this.dataKey]
              );
              let item = this.copyProperty(res.data);
              let idx;
              if (
            ...
            accept: () => {
              ...
              if (removeIds.length > 0) {
                this._apiService
                  .post('/api/adapter/execute', {
                    Method: { Method: 'POST' },
                    Url: '/api/website-tag/delete-website-tag',
                    Module: 'CMS',
    ...
    setTimeout(() => {
      ...
      arrRequest.push(
        this._apiService.post('/api/adapter/execute', {
          Method: { Method: 'GET' },
          Url: '/api/website-tag/get-by-id/' + row.tag_id,
          Module: 'CMS',
        })
      );
      Observable.combineLatest(arrRequest).subscribe((res: any) => {
        ...
        this.updateForm = new FormGroup({
          tag_name_l: new FormControl(this.website_tag.tag_name_l, [
            Validators.required,
            Validators.maxLength(1000),
          ]),
          tag_name_e: new FormControl(this.website_tag.tag_name_e, [
            Validators.required,
            Validators.maxLength(1000),
          ]),
          tag_description_l: new FormControl(
            this.website_tag.tag_description_l,
            [Validators.maxLength(1500)]
          ),
          tag_description_e: new FormControl(
            this.website_tag.tag_description_e,
            [Validators.maxLength(1500)]
          ),
          sort_order: new FormControl(this.website_tag.sort_order, [
            Validators.pattern(/^-?(0|[1-9]\d*)?$/),
          ]),
        });
        this.updateFormOriginalData = this.updateForm.getRawValue();
