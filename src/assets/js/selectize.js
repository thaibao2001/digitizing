...
(function (root, factory) {
  ...
  $.extend(Selectize.prototype, {
    ...
    onOptionSelect: function (e) {
      ...
      if ($target.hasClass('create')) {
        ...
        if (typeof value !== 'undefined') {
          ...
          //
          setTimeout(() => {
            if ($(self.$wrapper).parents('.move-next').length) {
              var $canfocus = $('[tabindex!="-1"]:focusable');
              var index = $canfocus.index(self.$control_input[0]) + 1;
    ...
    setActiveOption: function ($option, scroll, animate) {
      ...
      if (scroll || !isset(scroll)) {
        height_menu = self.$dropdown_content.parent().height();
        height_item = self.$activeOption.outerHeight(true);
        scroll = self.$dropdown_content.parent().scrollTop() || 0;
        y = self.$activeOption.offset().top - self.$dropdown_content.parent().offset().top + scroll;
        scroll_top = y;
        scroll_bottom = y - height_menu + height_item;

        if (y + height_item > height_menu + scroll) {
          self.$dropdown_content.parent().stop().animate({
    ...
    registerOption: function (data) {
      var key = this.settings && data ? hash_key(data[this.settings.valueField]) : null;
  ...
  Selectize.define('infinite_scroll', function (options) {
    ...
    self.onKeyUp = function (e) {
      var self = this;
      var debound = 1000;
      if (localStorage.getItem('app_setting')) {
        debound = JSON.parse(localStorage.getItem('app_setting')).typing_speed || 1000;
      }
      var value = self.$control_input.val() || '';
      if (self.settings.mode == 'single' && self.lastValue !== value) {
        self.clearOptions();
      }
      clearTimeout(timer);
    ...
    self.on('load', function () {
      self.$dropdown.on('scroll', function (event) {
        var scrollBottom = self.$dropdown[0].scrollHeight - (self.$dropdown.scrollTop() + self.$dropdown.height());
        if (scrollBottom < 200 && maxHeight < self.$dropdown[0].scrollHeight) {
          maxHeight = self.$dropdown[0].scrollHeight;
          page++;
          var query = JSON.stringify({
            search: self.lastValue,
            page: page
          });
          self.$dropdown.off('scroll');
          self.onSearchChange(query);
        }
      });
    });
  ...
  Selectize.define('enable_options', function (options) {
    var self = this;
    ...
    self.onFocus = (function () {
      ...
      return function () {
        ...
        $.each(options.enableOptions, function (index, option) {
